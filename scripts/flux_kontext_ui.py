        ...
        with gr.TabItem("单图像编辑"):
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    # LoRA刷新按钮事件处理
                    def refresh_portrait_lora_models():
                        """刷新LoRA模型列表"""
                    ...
                    portrait_lora_refresh.click(
                        fn=refresh_portrait_lora_models,
                        inputs=[],
                        outputs=[portrait_lora_model]
                    )
                    
                    # 单图像编辑选项
                    pose_edit_components = []
                    with gr.Row(equal_height=False):
                        ...
                        pose_edit_components.append(textbox1)
                    
                    # 单图像编辑文本框组件
                    pose_textboxes = list(pose_edit_components)
                ...
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        ...
                        def open_flux_kontext_output_dir():
                            """打开FLUX.1-Kontext图像编辑输出目录"""
                            output_dir = os.path.join(shared.data_path, "outputs", "flux-kontext")
                            os.makedirs(output_dir, exist_ok=True)
                            import subprocess
                            import platform
                        ...
                        open_output_dir_btn.click(fn=open_flux_kontext_output_dir, inputs=[], outputs=[])
                    
                    seed_info = gr.Textbox(label="使用的种子", interactive=False)
        # 添加双图像编辑标签页
        with gr.TabItem("双图像编辑"):
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    # 添加打开输出目录按钮
                    dual_open_output_dir_btn = gr.Button("打开输出目录")
                    dual_open_output_dir_btn.click(fn=open_flux_output_dir, inputs=[], outputs=[])
                ...
                # 修复单图编辑输入收集逻辑
                pose_inputs = []
                for i, textbox in enumerate(pose_textboxes):
                ...
                # 修复按钮点击事件绑定
                generate_btn.click(  # 修复"生成图像"按钮事件绑定
                ...
                portrait_generate_button.click(  # 修复"生成系列结果"按钮事件绑定
                    fn=on_generate_edit_series,
                    inputs=[
                        input_image, 
                        *pose_inputs,
                ...
                return {
                    # 批量编辑系列返回组件
                    "portrait_input_image": input_image,
                    "portrait_model_type": model_type,
                    "portrait_enable_cpu_offload": enable_cpu_offload,
                    "portrait_seed": seed,
                    "portrait_randomize_seed": randomize_seed,
                    "portrait_guidance_scale": guidance_scale,
                    "portrait_num_inference_steps": num_inference_steps,
