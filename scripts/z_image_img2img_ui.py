...
if __name__ != "__main__":
    # 当作为模块导入时，进行简单的初始化检查
    if Z_IMAGE_IMG2IMG_MODULE_AVAILABLE:
        if not model_exists:
            print("Z-Image-Turbo Img2Img：模型文件不存在，将在首次使用时提示用户下载")
    else:
        print("Z-Image-Turbo Img2Img：模块初始化失败，功能不可用")

def ensure_model_directory():
    """确保模型目录存在，如果不存在则创建并给出提示"""
    if not models_dir.exists():
        print(f"Z-Image-Turbo模型目录不存在: {models_dir}")
        print(f"请将Z-Image-Turbo模型文件下载到以下路径: {models_dir}")
        models_dir.mkdir(parents=True, exist_ok=True)
        return False
    elif not any(models_dir.iterdir()):
        print(f"Z-Image-Turbo模型目录为空: {models_dir}")
        print(f"请将Z-Image-Turbo模型文件放置在上述路径中")
        return False
    else:
        return True
...
def generate_image_img2img(init_image, prompt, negative_prompt, width, height, steps, cfg_scale, seed, sampler, batch_size=1,
    ...
    try:
        ...
        try:
            ...
            generator = torch.Generator().manual_seed(int(seed) if seed is not None else 42)
                ...
                if np.mean(img_array) < 5:  # 平均像素值低于5，认为是黑图
                    ...
                    if model_type == 'fp8':
                        ...
                        if np.mean(re_img_array) < 5:
                            ...
                            # 如果仍然是黑图，尝试使用不同的随机种子
                            alt_generator = torch.Generator().manual_seed(int(seed) + 12345 if seed is not None else 42)
                            re_images = img2img_pipe(
                                prompt=prompt,
                                negative_prompt=negative_prompt,
                                image=init_image_pil,
                        ...
                        alt_images = img2img_pipe(
                            ...
                            guidance_scale=float(actual_guidance),
                            generator=torch.Generator().manual_seed(int(seed) + 1000 if seed is not None else 42),
                            num_images_per_prompt=1,
                            output_type="pil"
                        ).images
            ...
            for idx, image in enumerate(processed_images):
                output_image_path = output_dir / f"{int(seed) if seed is not None else time.time()}_{idx}_img2img.png"
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():  # 左半边 - 参数设置
                init_image = gr.Image(label="输入图像", type="pil", height=400, width=400)
