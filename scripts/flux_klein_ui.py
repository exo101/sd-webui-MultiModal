...
def add_to_queue(task_type, *args):
    ...
    # 初始化 task_info，防止在未匹配任何条件时出现 UnboundLocalError
    task_info = None
    ...
    if task_type == 'sprite':
        ...
        task_summary = f"文生图: {args[1]}x{args[2]}, 提示词: {args[0][:30]}{'...' if len(args[0]) > 30 else ''}"
    else:
        ...
        try:
            if task_type == 'multi':
                ...
                result, status = generate_flux_klein_image(
                    ...
                    args[2],  # height - 在add_to_queue中是args[2]
                    args[1],  # width - 在add_to_queue中是args[1]
                    args[5],  # seed
                    args[6],  # model_type
                    args[7],  # batch_size
                    args[8],  # lora_enable
...
def get_detailed_queue_status():
    ...
    while not task_queue.empty():
        ...
        if task_type == 'txt2img':
            detail = f"任务{idx}: 文生图 - {task_info['params']['width']}x{task_info['params']['height']}"
    ...
    with gr.Tabs():
        with gr.TabItem("文生图"):
            ...
            add_to_queue_btn.click(
                fn=lambda prompt, width, height, steps, guidance_scale, seed, model_type, batch_size, lora_enable, lora_model, lora_weight: (
                    add_to_queue('txt2img', prompt, width, height, steps, guidance_scale, seed, model_type, batch_size, lora_enable, lora_model, lora_weight)
                ),
                inputs=[
                    prompt, width, height, 
        ...
        with gr.TabItem("局部重绘"):
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    outpaint_result_status = gr.Textbox(label="图像扩展状态信息", interactive=False)
