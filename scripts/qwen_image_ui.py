...
from modules.progress import create_task_id  # 导入创建任务 ID 的函数
...
def create_qwen_image_ui():
    try:
        ...
        text_to_image_button.click(
            fn=wrap_gradio_gpu_call(run_text_to_image, extra_outputs=[None, '']),  # 使用 wrap_gradio_gpu_call 直接包装原函数
            inputs=[
        ...
        # 发送到分镜功能
        if STORYBOARD_AVAILABLE:
            def send_qwen_gallery_to_storyboard(images):
                """将画廊中的图片发送到分镜助手"""
            ...
            send_to_storyboard_btn.click(
                fn=send_qwen_gallery_to_storyboard,
                inputs=[text_to_image_output],
                outputs=[send_status]
            )
