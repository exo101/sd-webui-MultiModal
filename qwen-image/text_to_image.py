...
def run_text_to_image(args_file):
    ...
    try:
        ...
        if model_file:
            print(f"用户选择的模型文件: {model_file}")  # 保留这一行用于显示用户选择的模型
        ...
        if is_sdnq_model and sdnq_model_path:
            try:
                ...
                # 对于SDNQ模型，跳过注意力优化以避免冲突
                # 同时设置transformer变量以避免后续代码出错
                transformer = pipeline.transformer
                
                # 关键修复：确保SDNQ模型的量化类型被正确识别
                # 防止出现"Unknown quantization type"错误
                if hasattr(pipeline, 'transformer') and pipeline.transformer is not None:
                    # 标记为量化模型，避免后续不必要的优化
        ...
        if not is_sdnq_model and model_path and model_path.exists():
            ...
            # 检测是否为量化模型
            is_quantized_model = hasattr(transformer, 'quantization_method') or \
                               any(hasattr(module, 'weight') and hasattr(module.weight, 'absmax') 
                                   for module in transformer.modules() 
                                   if 'Linear8bitLt' in str(type(module)) or 'Linear4bit' in str(type(module)))
            
            # 应用注意力优化（如果模型不是量化模型）
            apply_attention_optimizations(pipeline, is_quantized_model)
        ...
        try:
            ...
            if is_sdnq_model and sdnq_model_path:
                ...
                # 检查是否存在经过优化的transformer
                if hasattr(pipeline, 'transformer') and pipeline.transformer is not None:
                    # 检查transformer是否是量化模型
                    is_quantized_model = hasattr(pipeline.transformer, 'quantization_method') or \
                                       any(hasattr(module, 'weight') and hasattr(module.weight, 'absmax') 
                                           for module in pipeline.transformer.modules() 
                                           if 'Linear8bitLt' in str(type(module)) or 'Linear4bit' in str(type(module)))
                    
                    # 如果是量化模型，跳过自定义forward patching
                    if is_quantized_model:
                        # 直接调用pipeline，不应用额外的patch
                        images = pipeline(**generation_params).images
                    else:
                        # 使用原来的逻辑
                        images = pipeline(**generation_params).images
                else:
                    # 如果没有transformer，直接调用pipeline
                    images = pipeline(**generation_params).images
            print("图像生成完成")
