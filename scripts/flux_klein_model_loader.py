...
try:
    ...
    SDNQ_AVAILABLE = True
    print("[INFO] SDNQ库已成功导入，支持4bit量化模型")
except ImportError as e:
    print(f"[WARNING] 无法导入SDNQ库: {e}")
    print("如需使用SDNQ 4bit模型，请安装: pip install sdnq")
...
def _load_sdnq_model(model_type):
    ...
    """
    try:
        print(f"[INFO] 开始加载SDNQ 4bit模型: {model_type}")
        ...
        if not os.path.exists(model_id):
            print(f"[WARNING] 本地模型路径不存在: {model_id}")
            ...
            if "flux.2-klein-9b-sdnq-4bit-dynamic-svd-r32" in model_type.lower():
                ...
                print(f"[INFO] 切换到HuggingFace仓库ID: {model_id}")
            else:
                print(f"[ERROR] 无法找到模型: {model_type}")
                return None
        else:
            print(f"[INFO] 使用本地模型路径: {model_id}")
        
        print(f"[INFO] 使用模型ID/路径: {model_id}")
        ...
        print("[INFO] SDNQ模型加载完成!")
        return pipe
        
    except Exception as e:
        print(f"[ERROR] 加载SDNQ模型失败: {e}")
        import traceback
        traceback.print_exc()
