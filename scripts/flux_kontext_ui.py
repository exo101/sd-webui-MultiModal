...
def list_lora_models():
    """列出所有可用的LoRA模型"""
    lora_dir = os.path.join(os.path.dirname(__file__), "..", "FLUX.1-Kontext", "lora")
...
def load_lora_weights(pipe, lora_path, weight=0.5):
    ...
    try:
        ...
        if hasattr(pipe, 'load_lora_weights'):
            ...
            LOADED_LORA = lora_path
            LOADED_LORA_WEIGHT = weight
            print(f"成功加载LoRA模型: {lora_path} (权重: {weight})")
            return True
        else:
            print("当前模型不支持LoRA加载")
            return False
...
def find_existing_model(filename):
    ...
    model_path = os.path.join(
        ...
        '..', 'FLUX.1-Kontext', 'models', 'FLUX.1-Kontext-dev', 
        filename
    )
    abs_model_path = os.path.abspath(model_path)
    
    return abs_model_path if os.path.exists(abs_model_path) else None
...
def generate_edit_series(
    ...
    for img_idx, (input_image, (target_width, target_height)) in enumerate(zip(input_images, target_sizes)):
        ...
        for i, edit in enumerate(edits_to_generate):
            ...
            try:
                # 创建保存目录
                save_dir = os.path.join(os.path.dirname(__file__), "..", "FLUX.1-Kontext", "outputs")
        ...
        with gr.Column():
                ...
                def on_generate_edit_series(input_image, *args):
                    ...
                    if lora_enable and lora_model:
                        lora_dir = os.path.join(os.path.dirname(__file__), "..", "FLUX.1-Kontext", "lora")
                        lora_path = os.path.join(lora_dir, lora_model)
                        if os.path.exists(lora_path):
                            ...
                            if pipe is not None:
                                ...
                                # 加载新LoRA
                                load_lora_weights(pipe, lora_path, lora_weight)
                        else:
