...
def run_image_editing(args_file):
    ...
    try:
        ...
        # 显示生成参数摘要
        print(f"生成参数摘要:")
        print(f"  - 图像尺寸: {init_image.size if hasattr(init_image, 'size') else 'Unknown'}")
        print(f"  - 提示词: {prompt[:50]}{'...' if len(prompt) > 50 else ''}")
        print(f"  - 步数: {steps}")
        print(f"  - CFG比例: {cfg_scale}")
        print(f"  - 批次大小: {batch_size}")
        print(f"  - 种子: {args.get('seed', -1)}")
        print(f"  - 设备: {device}")

        # 生成图像 - 使用torch.inference_mode优化性能
        try:
            print("开始执行pipeline生成...")
            print("正在调用pipeline，这可能需要几分钟时间...")
            
            # 为避免长时间阻塞，我们可以设置一个线程来监控执行时间
            import threading
            import time
            
            # 定义一个标志来标记生成是否完成
            generation_completed = threading.Event()
            generation_result = [None]  # 使用列表来在闭包中存储结果
            generation_error = [None]   # 使用列表来在闭包中存储错误
            
            def run_generation():
                try:
                    with torch.inference_mode():
                        result = pipeline(**generation_params).images
            ...
            generation_completed.wait(timeout=timeout_seconds)
            
            if not generation_completed.is_set():
                print(f"图像生成超时 ({timeout_seconds}秒)，请检查模型和内存状态")
                return []
            else:
                # 等待线程结束
                ...
                if generation_error[0] is not None:
                    print(f"图像生成过程中发生错误: {str(generation_error[0])}")
                    import traceback
                    traceback.print_exc()
                    return []
                
                images = generation_result[0]
