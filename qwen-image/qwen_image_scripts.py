...
def run_image_editing(args_file):
    ...
    try:
        ...
        # 统一ControlNet启用条件：必须提供控制图像且模型不是"无"
        has_control_image = len(images) > 0 and images[0] is not None
        ...
        if controlnet_enable:
            # 预处理控制图像
            processed_control_image = preprocess_control_image(images[0], controlnet_preprocessor)
            if processed_control_image is None:
                print("控制图像处理失败")
        ...
        if len(images) > 0 and images[0]:
            try:
                init_image = load_image(images[0])
                # 确保图像是RGB模式
        ...
        images = pipeline(
            ...
            num_inference_steps=steps,
            true_cfg_scale=cfg_scale,
            control_image=processed_control_image if has_control_image else None,
            controlnet_conditioning_scale=controlnet_conditioning_scale if has_control_image else None,
            control_guidance_start=controlnet_start if has_control_image else None,
            control_guidance_end=controlnet_end if has_control_image else None,
