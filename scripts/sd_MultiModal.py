...
vision_model_names = [
    ...
    "huihui_ai/qwen3-vl-abliterated:2b",
]
...
def MultiModal_tab():
    with gr.Blocks(analytics_enabled=False) as ui:
        with gr.Tabs():
            ...
            # 添加 FLUX 系列标签页
            with gr.TabItem("7.FLUX加速系列图像生成与编辑"):
                with gr.Tabs():
                    # kontext图像编辑子标签页
            ...
            if 'QWEN_IMAGE_MODULE_AVAILABLE' in globals() and QWEN_IMAGE_MODULE_AVAILABLE:
                with gr.TabItem("8.Qwen Image图像生成与编辑"):
                    try:
                        with gr.Tabs():
                            with gr.TabItem("文生图"):
    ...
    return [(ui, "多模态插件15", "MultiModal_vision_tab")]
