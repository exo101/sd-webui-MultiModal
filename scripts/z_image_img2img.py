...
def generate_image_with_zimage_img2img(init_image, prompt, negative_prompt, width, height, steps, cfg_scale, seed, strength, batch_size, lora_enable, lora_model_1, lora_weight_1, lora_model_2, lora_weight_2, selected_model=None, enable_hr=False, hr_scale=2.0, hr_upscaler=None, hr_second_pass_steps=20, hr_resize_x=0, hr_resize_y=0, denoising_strength=0.7):
    """
    ...
    try:
        ...
        init_image = init_image.resize((first_pass_width, first_pass_height), Image.Resampling.LANCZOS)
        
        print(f"[INFO] 开始图生图生成...")
        print(f"[INFO] 参数: 提示词='{prompt[:50]}...', 尺寸={first_pass_width}x{first_pass_height}, 步数={steps}, 重绘强度={strength}, 批次={batch_size}")
        ...
        for i, image in enumerate(images):
            ...
            filename = f"zimage_img2img_{timestamp}_s{seed}_hr{'1' if enable_hr else '0'}_{i}.png"
        ...
        return f"图生图生成成功! 批次: {batch_size}, Seed: {seed}, HR: {'ON' if enable_hr else 'OFF'}", saved_paths
