...
def run_text_to_image(prompt, negative_prompt, width, height, steps, cfg_scale, 
    ...
    try:
        ...
        if "image_path" in output_info:
            ...
            print("文生图生成成功")
            return output_path, "生成成功", "生成完成"  # 简化返回值
        else:
...
def create_qwen_image_ui():
    try:
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        gr.Markdown(
                            ...
                            <strong>CFG参数说明</strong>: Lightning模型建议为1，普通模型建议为4，LoRA权重为1.5
                            </div>
                            """
                        )
        ...
        # 删除与Inpainting模型相关的组合处理函数 combined_control_image_handler
        ...
        text_to_image_button.click(
            ...
            outputs=[text_to_image_output, text_to_image_status]  # 移除text_to_image_info输出
        )
