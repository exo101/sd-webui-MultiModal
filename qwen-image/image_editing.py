    ...
    try:
        ...
        try:
            ...
            with torch.inference_mode():
                # 检查pipeline类型，根据实际类型进行处理
                pipeline_type = type(pipeline).__name__
                
                if sdnq_enable:
                    # SDNQ模型需要特殊处理，根据具体类型决定参数
                    inputs = {
                        "prompt": prompt,
                        "true_cfg_scale": cfg_scale,
                        "num_inference_steps": steps,
                        ...
                        "guidance_scale": generation_params.get("guidance_scale", 1.0)
                    }
                    
                    # 根据SDNQ模型类型决定是否包含image参数
                    if "QwenImageEditPlusPipeline" in pipeline_type:
                        # 编辑模型需要image参数
                        if controlnet_enable and processed_control_image is not None:
                            inputs["image"] = [init_image, processed_control_image]
                        else:
                            inputs["image"] = init_image
                    # 文生图模型(QwenImagePipeline)不需要image参数，所以不添加
                    
                    # 使用官方推荐的推理模式
                    with torch.inference_mode():
                        result = pipeline(**inputs)
                    images = result.images
                elif "QwenImageEditPlusPipeline" in pipeline_type:
                    # 对于标准Qwen编辑pipeline，使用正确的参数
                    image_input = generation_params.get("image", init_image)
                    images = pipeline(
                        image=image_input,
                        prompt=prompt,
                        true_cfg_scale=cfg_scale,
