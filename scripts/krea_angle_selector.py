...
def create_krea_angle_visualization_component(prompt_component):
    """
    为flux_krea_ui创建独立的多角度可视化选择器，使用3D相机视角界面
    """
    with gr.Group():
        gr.Markdown("### 3D相机控制调整视角")
        
        # 获取3D可视化界面的路径
        html_path = os.path.join(os.path.dirname(__file__), "camera_3d_view_krea.html")
        ...
        with gr.Row():
            ...
            <div style="position:relative; width:100%; height:400px;">
                <iframe id="camera-iframe-krea" name="camera-iframe-krea" 
        ...
        with gr.Row():
            ...
            apply_js = """
            async (prompt) => {
                console.log('Krea Apply button clicked, prompt:', prompt); // 添加错误检查
                
                // 获取iframe元素
                const iframe = document.getElementById('camera-iframe-krea');
                
                console.log('Krea Iframe element:', iframe); // 添加错误检查
                
                // 确保iframe已经加载完成
                if (!iframe || !iframe.contentWindow) {
                    console.error('无法访问krea iframe内容，可能尚未加载完成');
                ...
                console.log('Sending message to krea iframe...'); // 添加错误检查
                
                // 向iframe发送请求角度信息的消息
                iframe.contentWindow.postMessage({
                    type: 'GET_CURRENT_ANGLE_KREA'
                }, '*');
                
                console.log('Message sent to krea iframe, waiting for response...'); // 添加错误检查
                ...
                return new Promise((resolve) => {
                    ...
                    const timeoutId = setTimeout(() => {
                        ...
                        console.log(`等待krea角度数据超时 (${elapsed}ms)`);
                        alert(`错误：等待krea 3D视角界面响应超时 (${elapsed}ms)，请检查3D界面是否正常工作！`);
                        resolve([prompt]); // 返回原始提示词，不添加任何角度信息
                    }, 3000); // 增加超时时间到3秒
                    
                    const handleMessage = (event) => {
                        const elapsed = Date.now() - startTime;
                        console.log(`Received message from krea iframe after ${elapsed}ms:`, event.data); // 添加错误检查
                        
                        // 确保消息来源是我们的krea iframe
                        if (event.data.type === 'ANGLE_SELECTED_KREA') {
                            console.log('接收到krea角度数据:', event.data); // 调试信息
                            ...
                            console.log('Krea Parts after filtering:', parts); // 添加错误检查
                            console.log('Krea Event data received:', event.data); // 添加错误检查
                            
                            // 检查是否至少有一个角度信息（不仅仅是<sks>）
                            if (parts.length <= 1) {
                                console.log('没有接收到有效的krea角度数据');
                                ...
                                // 直接报错而不是使用默认值
                                alert('错误：未能从krea 3D视角界面获取到有效的角度数据，请检查3D界面是否正常工作！');
                                resolve([prompt]); // 返回原始提示词，不添加任何角度信息
                                return;
                            }
                            ...
                            console.log('Krea 新提示词:', newPrompt); // 调试信息
                            resolve([newPrompt]);
                        } else {
                            console.log('Received non-ANGLE_SELECTED_KREA message:', event.data); // 添加错误检查
                        }
                    };
