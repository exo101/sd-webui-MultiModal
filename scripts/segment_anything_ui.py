...
def save_segmentation_results(results, segmentation_type):
    ...
    try:
        ...
        save_dir = os.path.join(shared.data_path, "outputs", "segment-anything", segmentation_type)
...
def create_sam_segmentation():
    ...
    with gr.Row():
        ...
        def open_sam_output_dir():
            """打开图像分割输出目录"""
            from modules import shared
            output_dir = os.path.join(shared.data_path, "outputs", "segment-anything")
        ...
        open_output_dir_btn.click(fn=open_sam_output_dir, inputs=[], outputs=[])
