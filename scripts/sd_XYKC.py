...
# 导入新创建的模型支持模块
try:
    # 从XYKC_AI包导入Qwen3-VL-4B-Instruct支持模块
    sys.path.append(os.path.join(scripts_dir, ".."))
    from XYKC_AI.XYKC_AI_PyScripts.qwen3_vl_4b_instruct import (
        process_with_model as process_with_qwen3_vl_4b_instruct,
        is_model_available as is_qwen3_vl_4b_instruct_available,
        download_model as download_qwen3_vl_4b_instruct_if_needed,
        get_model_info as get_qwen3_vl_4b_instruct_info
    )
    QWEN3_VL_AVAILABLE = True
    
    # 定义扩展视觉模型名称列表
    extended_vision_model_names = ["qwen3-vl-4b-instruct"]
except ImportError as e:
    QWEN3_VL_AVAILABLE = False
    print(f"警告: 无法导入 Qwen3-VL-4B-Instruct 模型支持模块: {e}")
    # 如果导入失败，使用原始的模型列表
    extended_vision_model_names = []

# 尝试导入DeepSeek-OCR模型支持模块
try:
    sys.path.append(os.path.join(scripts_dir, ".."))
    from XYKC_AI.XYKC_AI_PyScripts.deepseek_ocr import (
        process_with_model as process_with_deepseek_ocr,
        is_model_available as is_deepseek_ocr_available,
        download_model as download_deepseek_ocr_if_needed,
        get_model_info as get_deepseek_ocr_info
    )
    DEEPSEEK_OCR_AVAILABLE = True
    
    # 扩展视觉模型名称列表
    extended_vision_model_names.append("deepseek-ocr")
except ImportError as e:
    DEEPSEEK_OCR_AVAILABLE = False
    print(f"警告: 无法导入 DeepSeek-OCR 模型支持模块: {e}")
...
class ModelProcessor:
    ...
    def run_model(args, script_path):
        """运行模型并获取输出"""
        # 特殊处理Qwen3-VL-4B-Instruct模型，不通过Ollama API调用
        if args[1] == "qwen3-vl-4b-instruct":
            # 直接调用Qwen3-VL-4B-Instruct模型处理函数
            if QWEN3_VL_AVAILABLE:
                # 检查是视觉任务还是文本任务
                if len(args) > 3 and args[0] == "vision":  # 有图像路径参数
                    image_path = args[3]
                    user_input = args[2]
                    return process_with_qwen3_vl_4b_instruct(image_path, user_input)
                else:  # 文本任务
                    user_input = args[2]
                    return process_with_qwen3_vl_4b_instruct(None, user_input)
            else:
                return "错误: Qwen3-VL-4B-Instruct 模型不可用"
        
        # 特殊处理DeepSeek-OCR模型，不通过Ollama API调用
        if args[1] == "deepseek-ocr":
            # 直接调用DeepSeek-OCR模型处理函数
            if DEEPSEEK_OCR_AVAILABLE:
                # 检查是视觉任务还是文本任务
                if len(args) > 3 and args[0] == "vision":  # 有图像路径参数
                    image_path = args[3]
                    user_input = args[2]
                    return process_with_deepseek_ocr(image_path, user_input)
                else:  # 文本任务
                    user_input = args[2]
                    return process_with_deepseek_ocr(None, user_input)
            else:
                return "错误: DeepSeek-OCR 模型不可用"
        ...
        if result.returncode != 0:
            ...
            try:
                error_output = error_output.encode('utf-8').decode('utf-8')
            except UnicodeDecodeError:
                try:
                    error_output = error_output.encode('utf-8').decode('gbk')
                except UnicodeDecodeError:
                    error_output = error_output.encode('utf-8').decode('utf-8', errors='ignore')
        ...
        try:
            ...
            try:
                output = output.encode('utf-8').decode('gbk')
            except UnicodeDecodeError:
                output = output.encode('utf-8').decode('utf-8', errors='ignore')
    ...
    def process_image(model_name, image_path, script_path, user_input, is_batch=False, save_dir=None):
        """处理单张或批量图片"""
        # 特殊处理Qwen3-VL-4B-Instruct模型
        if model_name == "qwen3-vl-4b-instruct" and QWEN3_VL_AVAILABLE:
            # 确保图像路径是绝对路径
            abs_image_path = os.path.abspath(image_path) if not os.path.isabs(image_path) else image_path
            # 直接调用Qwen3-VL-4B-Instruct模型处理函数
            result = process_with_qwen3_vl_4b_instruct(abs_image_path, user_input)
            if is_batch and save_dir:
                save_path = os.path.join(save_dir, os.path.splitext(os.path.basename(abs_image_path))[0]) + ".txt"
...
# 定义支持的视觉模型
base_vision_model_names = [
    "qwen2.5vl:latest",
    "qwen2.5vl:3b",    
    "llama3.2-vision:latest",
]

# 确保 qwen3-vl-4b-instruct 模型始终在列表中（如果可用则通过扩展列表添加，否则直接添加）
if QWEN3_VL_AVAILABLE and "qwen3-vl-4b-instruct" in extended_vision_model_names:
    # 扩展模块可用时，优先使用扩展列表中的模型顺序
    vision_model_names = extended_vision_model_names + [m for m in base_vision_model_names if m not in extended_vision_model_names]
else:
    # 扩展模块不可用时，手动将 qwen3-vl-4b-instruct 添加到基础列表前
    vision_model_names = ["qwen3-vl-4b-instruct"] + base_vision_model_names
